...
public class MLog_SQL {
	public static void createTable() {
		...
		try {
			Utils.cLog("sqlLog", "&aCreating table... If it dosen't esist alredy (MessagingLog)");
			st = SQL_Connection.getConnection().prepareStatement("CREATE TABLE IF NOT EXISTS " + SQL.pluginMessagingLog
					+ " ("
					+ "ID VARCHAR(5),TIME VARCHAR(100),FROMSERVER VARCHAR(16),TOSERVER VARCHAR(16),TYPE VARCHAR(30)"
					+ ",MESSAGE TEXT, PRIMARY KEY (ID))");
			st.executeUpdate();
			Utils.cLog("sqlLog", "&aCreated table... If it dosen't esist alredy (MessagingLog)");
	...
	public static void sendMessage(String server, String type, String message) {
		if (SQL.enabled) {
			PreparedStatement st;
			try {
				st = SQL_Connection.getConnection().prepareStatement("INSERT INTO " + SQL.pluginMessagingLog
		...
		if (SQL.enabled) {
			try {
				...
				st = SQL_Connection.getConnection()
						.prepareStatement("SELECT * FROM " + SQL.pluginMessagingLog + " WHERE ID=?");
				st.setString(1, name);
